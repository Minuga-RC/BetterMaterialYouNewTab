//import '/home/ubuntu/BetterMaterialYouNewTab/node_modules/@material/web/all.js';
//import '/home/ubuntu/BetterMaterialYouNewTab/node_modules/@material/web/icon/icon.js'
...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    const useGPSButton = document.getElementById("useGPSbtn");
    const saveProxyButton = document.getElementById("saveproxybtn");
    const resetbtn = document.getElementById("resetsettingsbtn");
    const resetapi = document.getElementById("resetAPIbtn");
    ...
    if (savedApiKey) {
        ...
        resetapi.style.display = "block";
        saveAPIButton.style.display = "none";
    } else {
        resetapi.style.display = "none";
        saveAPIButton.style.removeProperty("display");
    }

    // Add event listener to reset API but  ton
    resetapi.addEventListener("click", () => {
        showDialog({
            headline: 'Reset Weather API Key?',
            icon: 'warning',
            textBtn: 'Yes',
            tonalBtn: 'No',
            formText: "Are you sure you want to reset the Weather API key? to the default key",
            onTextBtnClick: () => {
    ...
    userAPIInput.addEventListener("keydown", (event) => handleEnterPress(event, "saveAPIbtn"));
    userLocInput.addEventListener("keydown", (event) => handleEnterPress(event, "saveLocbtn"));
    userProxyInput.addEventListener("keydown", (event) => handleEnterPress(event, "saveproxybtn"));
    ...
    saveAPIButton.addEventListener("click", () => {
        ...
        showDialog({
            ...
            tonalBtn: 'OK',
            formText: "API Key saved successfully. Page need to be Refreshed to apply changes.",
            onTextBtnClick: () => {
                return;
            },
            onTonalBtnClick: () => {
    ...
    resetbtn.addEventListener("click", () => {
        showDialog({
            ...
            textBtn: 'Reset',
            tonalBtn: 'Cancel',
            formText: translations[currentLanguage]?.confirmRestore || translations["en"].confirmRestore,
            onTextBtnClick: () => {
                localStorage.clear();
                location.reload();
            },
    ...
    saveProxyButton.addEventListener("click", () => {
        ...
        userProxyInput.value = proxyurl;
        showDialog({
            headline: 'Proxy Saved',
            icon: 'done',
            textBtn: 'I will Refresh later',
            tonalBtn: 'OK',
            formText: "Proxy URL saved successfully. Page will Refresh to apply changes.",
            onTextBtnClick: () => {
                return;
            },
            onTonalBtnClick: () => {
    ...
    async function fetchWeather(location) {
        ...
        try {
            ...
            showDialog({
                ...
                icon: 'error',
                textBtn: 'Reset API Key',
                tonalBtn: 'Cancel',
                formText: "Failed to fetch weather data. Please check your API key and location or internet connection.\n\n Click 'Reset API Key' to reset the Weather API key to the default value.",
                onTextBtnClick: () => {
                    localStorage.removeItem("weatherApiKey");
                        setTimeout(() => {
                            showDialog({
                                headline: 'Successfully Reset API Key',
                                icon: 'done',
                                textBtn: 'I will Refresh later',
                                tonalBtn: 'OK',
                                formText: "The Weather API key has been reset to the default value. Page need to be Refreshed to apply changes.",
                                onTextBtnClick: () => {
                                    return;
                                },
                                onTonalBtnClick: () => {
                ...
                showAlert("Weather Data Unavailable", "Failed to fetch weather data. Please check your API key and location or internet connection.");
            }
        }
    }
    ...
    function showAlert(title, description) {
        ...
        if (menuBar.style.display === "none") {
            ...
            document.getElementById('alertbodyheadline').innerText = title;
            document.getElementById('alertbodyform').innerText = description;
            alert.show();
        }
        else {
            const alert = document.getElementById("menualertbody");
            document.getElementById('menualertbodyheadline').innerText = title;
            document.getElementById('menualertbodyform').innerText = description;
            alert.show();
        }
    }
    function showDialog({ headline, icon, textBtn, tonalBtn, formText, onTextBtnClick, onTonalBtnClick, onIgnored }) {
        const menuBar = document.getElementById("menuBar");
        if (menuBar.style.display === "none") {
            const resetdialog = document.getElementById("resetdialog");
            document.getElementById('dialogheadline').innerText = headline;
            document.getElementById('dialogicon').innerText = icon;
            document.getElementById('dialogtextbtn').innerText = textBtn;
            document.getElementById('dialogtonaltbtn').innerText = tonalBtn;
            document.getElementById('form').innerText = formText;
            resetdialog.show();

            // Remove any existing event listeners
            resetdialog.removeEventListener('close', handleDialogClose);

            // Define the event handler function
            function handleDialogClose() {
                const output = resetdialog.returnValue;
                if (output === 'txtbtn') {
                    onTextBtnClick();
                } else if (output === 'tonalbtn') {
                    onTonalBtnClick();
                } else {
                    onIgnored();
                }
            ...
            resetdialog.addEventListener('close', handleDialogClose);
        } else {
            const resetdialog = document.getElementById("menuresetdialog");
            document.getElementById('menudialogheadline').innerText = headline;
            document.getElementById('menudialogicon').innerText = icon;
            document.getElementById('menudialogtextbtn').innerText = textBtn;
            document.getElementById('menudialogtonaltbtn').innerText = tonalBtn;
            document.getElementById('menuform').innerText = formText;
            resetdialog.show();
    ...
    // Show welcome alert only on the first load
    if (!localStorage.getItem("firstLoadCompleted")) {
        showWelcomeDialog();
    }
    function showWelcomeDialog() {
        showDialog({
            headline: 'Welcome to Better Material You NewTab',
            icon: 'info',
            textBtn: 'Privacy Policy',
            tonalBtn: 'Agree & Continue',
            formText: `This is the homepage of your browser. You can customize it by clicking on the pencil icon on the bottom right corner.\n\nBy clicking 'Agree & Continue', you agree to our Privacy Policy.`,
            onTextBtnClick: () => {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    bookmarkViewGrid.addEventListener("click", function () {
        if (!bookmarkGridCheckbox.selected) bookmarkGridCheckbox.click();
    });
            ...
            if (node.children && node.children.length > 0) {
                ...
                deleteButton.addEventListener("click", function (event) {
                    ...
                    showDialog({
                        ...
                        textBtn: 'Delete',
                        tonalBtn: 'Cancel',
                        formText: `${(translations[currentLanguage]?.deleteBookmark || translations["en"].deleteBookmark)} "${node.title || node.url}"?`,
                        onTextBtnClick: () => {
...
function updatedigiClock() {
    ...
    greetingCheckbox.selected = isGreetingEnabled;
...
proxybox.addEventListener("input", () => {
    ...
    if (proxybox.value) {
        ...
        if (savedProxy) {
            ...
            proxybtn.style.removeProperty("display");
...
apifield.addEventListener("input", () => {
    ...
    if (apifield.value) {
        ...
        if (savedApiKey) {
            resetapi.style.display = "none";
            apisave.style.removeProperty("display");
        }
    } else {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    userTextCheckbox.selected = isUserTextVisible;
    ...
    userTextCheckbox.addEventListener("change", () => {
        const isVisible = userTextCheckbox.selected;
    ...
    userTextDiv.addEventListener("focus", function () {
        if (userTextDiv.value === userTextDiv.dataset.placeholder) {
            userTextDiv.value = "";  // Clear the placeholder when focused
        }
    });

    // Restore placeholder if the user leaves the div empty after editing
    userTextDiv.addEventListener("blur", function () {
        if (userTextDiv.value === "") {
            userTextDiv.value = userTextDiv.dataset.placeholder;  // Show the placeholder again if empty
        }
    });
});
...
micIconCheckbox.selected = !isMicIconVisible; // selected hides the mic icon
if (isMicIconVisible) {
...
micIconCheckbox.addEventListener("change", () => {
    const isChecked = micIconCheckbox.selected;
    toggleMicIconVisibility(!isChecked); // selected hides the mic icon
...
async function applyRandomImage(showConfirmation = true) {
    showDialog({
        headline: "Daily Wallpaper!",
        icon: 'info',
        textBtn: 'No',
        tonalBtn: 'Yes',
        formText: translations[currentLanguage]?.confirmWallpaper || translations["en"].confirmWallpaper,
        onTextBtnClick: () => {
           return;
        },
        onTonalBtnClick: () => {
...
async function applyranimage(showConfirmation = true){
    if (showConfirmation && !secondconfirmation) {
        return;
    }
    try {
        ...
        showAlert("Something went wrong :(", `Error fetching random image: ${error.message}`);
    }
}
...
document.getElementById("clearImage").addEventListener("click", function () {
    loadImageAndDetails()
        .then(([blob]) => {
            if (!blob) {
                showAlert("Something went wrong :(", translations[currentLanguage]?.Nobackgroundset || translations["en"].Nobackgroundset);
                return;
            }
            const confirmationMessage = translations[currentLanguage]?.clearbackgroundimage || translations["en"].clearbackgroundimage;
            showDialog({
                headline: "Clear Background Image?",
                icon: 'warning',
                textBtn: 'Yes',
                tonalBtn: 'No',
                formText: confirmationMessage,
                onTextBtnClick: () => {
...
async function backupData() {
    try {
        ...
        const fileName = `BMYNT_Settings_${formattedDate}.json`;
        ...
        showAlert("Something went wrong :(", translations[currentLanguage]?.failedbackup || translations["en"].failedbackup + error.message);
    }
}
    ...
    reader.onload = async (e) => {
        try {
            ...
            if (!isValidBackupFile(backup)) {
                showAlert("Something went wrong :(", translations[currentLanguage]?.invalidBackup || translations["en"].invalidBackup);
                return;
            }
            await restoreData(backup);
            showDialog({
                headline: translations[currentLanguage]?.restorecompleted || translations["en"].restorecompleted,
                icon: 'done',
                textBtn: 'I will Refresh later',
                tonalBtn: 'OK',
                formText: "Your settings have been restored successfully. Page refresh Required to apply changes.",
                onTextBtnClick: () => {
                    return;
                },
                onTonalBtnClick: () => {
            ...
            showAlert("Something went wrong :(", translations[currentLanguage]?.restorefailed || translations["en"].restorefailed + error.message);
        }
    };
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const PLACEHOLDER_SHORTCUT_URL = "https://rhythmcorehq.com/";
    ...
    function applyShortcut(shortcut) {
        ...
        if (!isValidUrl(url)) {
            showAlertDialog("Invalid URL", "Please enter a valid URL with http or https protocol.");
            url = "https://rhythmcorehq.com/404";
        }
    ...
    function getFallbackFavicon(urlString) {
        ...
        } else if (urlString === "https://rhythmcorehq.com/404") {
    ...
    if (localStorage.getItem("showShortcutSwitch")) {
        ...
        document.getElementById("shortcut_switchcheckbox").selected = isShortCutSwitchEnabled;
    ...
    greetingCheckbox.selected = localStorage.getItem("greetingEnabled") === "true";
    ...
    greetingCheckbox.addEventListener("change", () => {
        localStorage.setItem("greetingEnabled", greetingCheckbox.selected);
        updatedigiClock();
    });
    ...
    useproxyCheckbox.addEventListener("change", function () {
        if (useproxyCheckbox.selected) {
            ...
            showDialog({
                headline: 'Please Use Your Own Proxy',
                icon: 'warning',
                textBtn: 'Ok',
                tonalBtn: 'Cancel',
            ...
            // If the checkbox is unselected, disable the proxy
            saveCheckboxState("useproxyCheckboxState", useproxyCheckbox);
            proxyinputField.classList.add("inactive");
            saveActiveStatus("proxyinputField", "inactive");
        }
    ...
    iconStyle.textContent = adaptiveIconToggle.selected ? ADAPTIVE_ICON_CSS : "";
    ...
    bookmarksCheckbox.addEventListener("change", function () {
        ...
        if (isFirefox && browser.permissions && isDesktop) {
            ...
            showAlert("Something went wrong :(", translations[currentLanguage]?.UnsupportedBrowser || translations["en"].UnsupportedBrowser);
            bookmarksCheckbox.selected = false;
            saveCheckboxState("bookmarksCheckboxState", bookmarksCheckbox);
            return;
        }
        if (bookmarksPermission !== undefined) {
            if (bookmarksCheckbox.selected) {
    ...
    // Select all primary tabs and panels
...
//handleTabClick({ currentTarget: document.getElementById('shortcuts-panel') });
    function handleTabClick(event) {
        closeshortcuteditmenu();
        panels.forEach(panel => (panel.hidden = true));
        tabs.forEach(tab => tab.removeAttribute('active'));
        //secondaryPanels.forEach(p => (p.hidden = true));
        //secondaryTabs.forEach(t => t.removeAttribute('active'));
        secondaryPanels2.forEach(p => (p.hidden = true));
        //secondaryTabs2.forEach(t => t.removeAttribute('active'));

        const selectedTab = event.currentTarget;
        const selectedPanelId = selectedTab.id.replace('-tab', '-panel');
        selectedTab.setAttribute('active', '');
        if (selectedTab.id === 'settings-tab') {
            generalpanel.hidden = false;
            advancedpanel.hidden = true;
            generaltab.setAttribute('active', '');
            advancedtab.removeAttribute('active');
        }else if (selectedTab.id === 'style-tab') {
            appearancepanel.hidden = true;
            homepanel.hidden = false;
            hometab.setAttribute('active', '');
            appearancetab.removeAttribute('active');
        }
    ...
    // Secondary tabs group 1
    const secondaryTabs = document.querySelectorAll('[role="secondary-tab1"]');
    const secondaryPanels = document.querySelectorAll('[role="tabpanel2"]');

    function handleSecondaryTabClick(event) {
        secondaryPanels.forEach(panel => (panel.hidden = true));
        secondaryTabs.forEach(tab => tab.removeAttribute('active'));
    ...
    function loadActiveSecondaryTab1() {
        const storedTabId = localStorage.getItem('activeSecondaryTab1');
        if (storedTabId && document.getElementById(storedTabId)) {
            document.getElementById(storedTabId).click();
        } else {
            secondaryTabs[0].click();
        }
    }
    loadActiveSecondaryTab1();

    // Secondary tabs group 2
    const secondaryTabs2 = document.querySelectorAll('[role="secondary-tab2"]');
    const secondaryPanels2 = document.querySelectorAll('[role="tabpanel3"]');

    function handleSecondaryTab2Click(event) {
        secondaryPanels2.forEach(panel => (panel.hidden = true));
        secondaryTabs2.forEach(tab => tab.removeAttribute('active'));

        const selectedTab2 = event.currentTarget;
        const selectedPanelId2 = selectedTab2.id.replace('-tab', '-panel');
        selectedTab2.setAttribute('active', '');
        document.getElementById(selectedPanelId2).hidden = false;

    }

    secondaryTabs2.forEach(tab => {
        tab.addEventListener('click', handleSecondaryTab2Click);
    });

    function loadActiveSecondaryTab2() {
        const storedTabId2 = localStorage.getItem('activeSecondaryTab2');
        if (storedTabId2 && document.getElementById(storedTabId2)) {
            document.getElementById(storedTabId2).click();
        } else {
            secondaryTabs2[0].click();
        }
    }
    loadActiveSecondaryTab2();
